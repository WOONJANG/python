...
#pd.read_excel("person.xls") 로 로드해오면 가능
#예) usercols='A,B,D:X'
...
#iloc(xls, xlsx전용) : 행 번호에 맞춰서 해당 행에 있는 모든 데이터를 가져오는 함수입니다.
# print(data.iloc[1])
#컬럼값만 출력(data.columns) : 배열로 컬럼 이름만 출력
#data.columns
'''
a_data=data.iloc[0]
...
mpt.barh(data.columns, -m_data//1000,label="남성인구") #남성데이터
mpt.barh(data.columns, w_data//1000,label="여성인구") #여성데이터
mpt.legend()
